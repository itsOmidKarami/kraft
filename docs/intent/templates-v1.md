...
child scope's cap SHALL NOT exceed its parent's: one that does SHALL be refused
when the chain loads and when a work item is filed or its override set, naming
both scopes, and a work item's own cap SHALL only tighten. A gate's timeout
