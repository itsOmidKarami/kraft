...
left partial work. A fallback entry SHALL keep from the task's own launch what
it omits, and SHALL NOT carry the work item's or the node's model and effort
...
An agent task with no fallback list, or an empty one, SHALL launch, park on a
rate limit and stop on an unavailable harness exactly as it would without the
feature, and SHALL NOT consult rate-limit memory. No shipped task declares a
fallback list.
...
disabled or carries a default Kraft cannot apply, or whose executable is not
...
Materialization SHALL refuse an agent task any of whose fallback entries names
a harness outside the task's `allowed_harnesses`.
enforced-by: tests/templates/test_fallback_entries.py::test_a_fallback_harness_outside_allowed_harnesses_fails_materialization
...
A fallback entry SHALL be refused at load when it names none of harness, model
and effort, or carries an unknown key; a gate's review task SHALL NOT declare a
fallback list. A harness save SHALL be refused when it leaves a fallback entry
of a chain's task unable to pair with its harness, naming the task and the
entry's index.
