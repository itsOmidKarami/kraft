...
does not define. A launch against the item's own repository SHALL find its
frozen text by the item's own repo as recorded at intake, not only by
`repos.yaml`'s current `path:` for it, so that a hand-edited path does not
silently drop an in-flight item's frozen steering for that repository; a
repository named nothing at intake still gets nothing, whatever `repos.yaml`
...
origin: src/kraft/worker/sandbox.py -- Kraft-dshto, Kraft-nx4id, Kraft-69rwp; review-g1 finding 1.
...
WHEN `kraft.mr_rebase` runs for a workspace item, the system SHALL first rebase
each changed member (one `open_mr` would publish) onto that member's own base
branch, under the same time cap, and SHALL leave an unchanged member alone.
WHEN a member moved and the root had committed the member's old head, the
system SHALL commit the root's pointer to the rebased member before rebasing
