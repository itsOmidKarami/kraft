...
origin: src/kraft/templates/models.py §MaterializedChain.policy_for -- the chain's policy (instance → repository → chain, folded at materialization) with each enclosing node, step and task override applied, then the work item's own override (`policy.WorkItemPolicy`, Kraft-ab1bh): item-wide, then each enclosing path's. It is applied last rather than between the repository and the chain, so its operational values win over the template's (`work-item-policy-may-exceed-default-ceilings-within-admin-maximum`); a safety value only tightens either way, and one the chain already narrowed is refused rather than widened.
