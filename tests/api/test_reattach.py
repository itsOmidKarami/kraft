...
def test_reattach_adopts_running_agent(tmp_path):
    ...
    # is what actually gets `KRAFT_FAKE_CLAUDE*` from this real daemon
    # subprocess's own environment into the worker it launches -- the
    # autouse fixture in conftest.py that does this for in-process tests has
    # no effect here, since the daemon runs as a separate `python -m kraft`.
