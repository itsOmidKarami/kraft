...
def test_work_item_documents_carry_the_run_their_session_came_from(client, repo):
    ...
    sessions = {s["id"]: s for s in client.get(f"/api/work-items/{wid}").json()["worker_sessions"]}
...
def test_log_jsonl_and_plain_text_are_both_served(client, repo):
    wid = _completed_item(client, repo)
...
def test_log_follow_streams_the_lines_then_ends(client, repo):
    wid = _completed_item(client, repo)
    sid = client.get(f"/api/work-items/{wid}").json()["worker_sessions"][0]["id"]
...
def test_log_follow_keeps_streaming_a_pending_session(client, repo, tmp_path):
...
def test_open_document_reports_501_when_no_editor_is_installed(client, repo, monkeypatch):
...
def test_open_document_refuses_a_document_with_no_file_in_the_repo(client, repo, monkeypatch):
    ...
    assert client.get(f"/api/documents/{doc['document_id']}").json()["origin"] == ("event_ingest")
...
def test_retry_is_refused_on_an_item_that_is_not_stopped(client, repo):
    wid = _completed_item(client, repo)
...
def test_retry_restarts_a_stopped_node_that_has_no_fix_loop(client, repo):
...
def test_log_lines_carry_the_time_the_parent_saw_them(client, repo):
...
def test_open_worktree_launches_the_editor_on_the_checkout(client, repo, monkeypatch):
    wid = _completed_item(client, repo)
...
def test_bead_search_is_live_and_quiet_on_an_empty_query(client):
...
@pytest.mark.api_client(peer=("10.0.0.5", 54321))
def test_open_worktree_is_refused_for_a_non_loopback_client(client, repo, monkeypatch):
    _as_authenticated_lan_peer(client, monkeypatch)
    wid = _completed_item(client, repo)
...
def test_open_document_refuses_a_document_path_outside_its_repo(client, repo, monkeypatch):
...
def test_retry_rebases_the_worktree_onto_a_moved_head(client, repo):
