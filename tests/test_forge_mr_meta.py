...
def test_mr_body_leads_with_the_description(tmp_path):
...
def test_mr_body_without_meta_is_the_old_default_body(tmp_path):
    plain = mr_ops.mr_body(WID, "kraft/abc", ("first commit",))
    assert plain == (
        f"Opened by Kraft for work item {WID}.\n\n"
        "Commits on this branch:\n\n- first commit\n\n"
        "Branch `kraft/abc`. Review the diff and the pipeline before merging."
    )


def test_mr_body_truncates_a_runaway_description():
