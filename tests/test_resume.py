...
from support.harness import isolated_bd, v1_named_chain, v1_seeded_chain

from kraft import builtins, events, executor, store
...
async def test_resume_from_verify_with_env_and_impl_done(
    ...
    wt = await _prepare(database, run_dirs, repo, wid)
    impl_node = executor.chain_of(row).chain.nodes[0]
    assert await executor.walk_node(database, run_dirs, wid, impl_node, row, wt) == "ok"
...
async def test_resume_no_session_for_current_node_dispatches_fresh(
    tmp_path, monkeypatch, database, run_dirs, repo
):
    ...
    impl_node = executor.chain_of(row).chain.nodes[0]
    await database.write(lambda c: store.load_chain(c, wid, "implementation"))
    await executor.walk_node(database, run_dirs, wid, impl_node, row, wt)
    ...
    await database.write(
        lambda c: c.execute(
            "DELETE FROM worker_sessions WHERE work_item_id = ? AND node_id = 'implementation'",
            (wid,),
        )
    )
...
async def test_resume_current_node_failed_session_is_needs_human(
...
async def test_resume_still_gives_a_node_the_repair_its_template_declared(
    tmp_path, database, run_dirs, repo
):
...
async def test_resume_awaits_adopted_task_before_reading_status(tmp_path, database, run_dirs, repo):
    ...
    sid = "s-impl-running"
    (run_dirs.worktrees / wid).mkdir(parents=True, exist_ok=True)
    await database.write(lambda c: store.load_chain(c, wid, "implementation"))
    await database.write(lambda c: store.enter_node(c, wid, "implementation"))
    await database.write(
        lambda c: store.create_session(
...
async def test_resume_before_gate_approval_still_re_requests_gate(
    tmp_path, monkeypatch, database, run_dirs, repo
):
    monkeypatch.setenv("KRAFT_FAKE_AGENT", "fix")
    tracker = isolated_bd(tmp_path)
