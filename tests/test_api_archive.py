...
from support.api import _client, _poll_events, _post_default, _set_status
from support.harness import make_repo


def test_archive_reclaims_the_worktree_and_keeps_status(tmp_path, monkeypatch):
...
def test_archive_refuses_an_active_item(tmp_path, monkeypatch):
    repo = make_repo(tmp_path)
    with _client(tmp_path, monkeypatch) as client:
        wid = _post_default(client, repo)
        _poll_events(client, wid, "gate_requested")
...
def test_restore_puts_it_back_and_refuses_a_non_archived_item(tmp_path, monkeypatch):
    repo = make_repo(tmp_path)
    with _client(tmp_path, monkeypatch) as client:
        wid = _post_default(client, repo)
        _poll_events(client, wid, "gate_requested")
...
def test_list_excludes_archived_by_default_and_archived_true_returns_only_them(
...
def test_archived_list_needs_include_abandoned_for_abandoned_items(tmp_path, monkeypatch):
