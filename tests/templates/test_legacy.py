...
SKILLS_DIR = Path(__file__).resolve().parents[2] / "src" / "kraft" / "skills"
...
def test_default_templates_mr_checks_has_the_ci_fix_loop_shape(tmp_path):
    ...
    real_dir = Path(__file__).resolve().parents[2] / "templates"
...
def test_chain_review_splice_fixture_matches_the_shared_cases():
    ...
    cases = json.loads(
        (Path(__file__).resolve().parents[1] / "fixtures" / "chain_review_splice.json").read_text()
    )["cases"]
