---
title: Install
description: Install, verify, upgrade, and uninstall Kraft with uv, the install script, or Homebrew.
---


Install Kraft with one of three methods: [uv](#install-with-uv), the
[install script](#install-with-the-script), or [Homebrew](#install-with-homebrew-macos).
Then run `kraft` to serve the board at `http://127.0.0.1:8765`.
...
Building Kraft from source also needs Node and `just`. CI builds with Node 22;
the frontend does not pin a minimum. Building from source is covered in
...
install with the `vector` extra. It downloads a model of about 130 MB on the
first search.
...
This applies to the uv and script installs. This page does not cover enabling
vector search on a Homebrew install.

## Connect your agent

The preferred way to wire Kraft into a coding agent is the
[Claude Code plugin marketplace](/reference/agent-integration#installing-the-plugin).
The plugin installs the MCP server and the `/kraft:*` skills and keeps them
updated. Then run `/kraft:onboard` in a repo: it connects the repo and checks
its config. The `kraft` command must be on your `PATH` for either to work. See
...
Binding off loopback requires a password. Set one in Settings, then Access,
while still on `127.0.0.1`. The process refuses to start on a LAN address
without one. See [Remote access](/reference/remote-access).
