...
see [CLI → Service and admin](/reference/cli#service-and-admin).
...
| `kind` | `agent`. |
| `harness` | The [harness profile](/reference/harnesses) it runs on — an id from `harnesses.yaml`. A disabled or missing profile stops the task for a human; nothing substitutes another unless the task declares `fallback`. |
...
checks all of its tasks together ([Concepts](/guide/concepts#node)).
...
[Concepts](/guide/concepts#chain) for the node schema, with the shipped `default`
...
|---|---|---|
| `allowed_tools` | only narrows | The [permission gate](/reference/permissions) answers a worker's ask from it, and it is passed as `--allowedTools`. Unset (no layer sets it) allows every tool; `[]` allows none. A harness with no tool-list capability (codex, gemini) refuses to launch under one rather than run unrestricted. Allowing `Bash` allows its read-only use without a gate ask: Claude's `manual` mode runs a read-only shell command (`cat`, `ls`, `git status`) itself and asks the gate only about the rest, so a read-only command can read any file the worktree holds, gitignored ones included. |
| `deny_tools` | only accumulates | Denied by the [permission gate](/reference/permissions) and passed as `--disallowed-tools`, on top of `allowed_tools`. |
| `grants` | only accumulates; a workspace meet keeps only what every repository grants; the work item's own override can drop one, never add one | Named operations a task is guaranteed: `git-commit`, `git-rebase`, `git-push`. The [permission gate](/reference/permissions#grants) allows a call that is exactly one plain git invocation of a granted subcommand, even outside `allowed_tools`, unless `deny_tools` denies it. That allow is the gate's; the CLI may still not run the call — Cursor's classifier can refuse it, and Claude under an allowlist without `Bash` has no shell tool to make it with (see [where a grant takes effect](/reference/permissions#grants)). |
...
same name, unless the task itself declares a [`fallback:`](/reference/harnesses#fallback)
list.
...
[agent profiles](/reference/harnesses#agent-profiles), the model tiers a task selects
...
| `fallback` | Optional. The tier's default [fallback list](/reference/harnesses#fallback), for a task selecting it that sets none of its own. Entries as on a task; a `profile:` an entry names must be defined here, and its own list is not followed. |
