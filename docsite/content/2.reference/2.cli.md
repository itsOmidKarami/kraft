...
The fields are the ones `policy.yaml` explains (see [Configuration](/reference/configuration)):
...
kraft admin permission-hook HARNESS [--fail-closed]  # what a Cursor or Codex worker's pre-tool hook runs; see The permission gate
```
...
`library.yaml` as steering profiles ([Configuration](/reference/configuration#repository-steering)). `policy.yaml` starts from the V1 default and
