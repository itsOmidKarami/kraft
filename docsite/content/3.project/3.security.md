...
- **Host allowlist.** A non-loopback bind also checks incoming requests'
  ...
  does not authorize an arbitrary hostname. See [Remote access](/reference/remote-access)
...
- **An agent cannot start work or approve its own gate.** Everything a coding
  ...
  convention. See [Agent integration](/reference/agent-integration).
- **Worker environment is allowlisted, not inherited.** A worker's process
  ...
  shell happened to have set. See [Configuration](/reference/configuration#reposyaml-connected-repos).
