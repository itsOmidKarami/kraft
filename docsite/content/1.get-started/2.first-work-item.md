...
- `uv` or Homebrew (see [Install](/get-started/install)).
- At least one supported coding-agent CLI, installed and logged in. See
  [Harnesses](/reference/harnesses) for the list.
- For steps 5 and 6 only: the repo needs a GitHub or GitLab remote, with `gh`
  or `glab` logged in, because Kraft opens, watches and merges the merge
  request through it. The `quick-task` chain has no such gates.
...
[Agent integration](/guides/agent-integration) if that is closer to how you
already work.
::
...
way. See [Install](/get-started/install) for that path and the install script.
...
`package.json`, a `pyproject.toml`). The command prints the `test_command` it
found. Open the repo's entry in `~/.kraft/templates/repos.yaml` (or Settings, then
Repos) and check all three before you trust them. Set `setup_command: ""` if the
repo needs no preparation. See [Configuration](/reference/configuration/repos).
...
`--autostart`. Nothing has spent a token yet. See [Concepts](/concepts/vocabulary#work-item).
