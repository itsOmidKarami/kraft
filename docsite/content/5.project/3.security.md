...
- **An agent cannot file work and start it in one step, or approve its own
  gate.** Everything a coding agent files through the MCP tools or
  `kraft item create` lands paused; a person starts it, from the board or by
  asking their agent to resume it. `kraft item create --autostart` is a person's shortcut
