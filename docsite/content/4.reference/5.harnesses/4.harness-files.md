...
| Key | Meaning |
|---|---|
| `command_resume` | The argv prefix for a resume, when it differs from `command` (see [below](#constraints-and-alternate-bindings)). |
...
- `values:` is a list of `re.fullmatch` patterns. A value outside them fails at
  load, not at launch.
- `always:` is the value Kraft uses when a binding supplies none. For a list
