...
- The task's profile **name** is frozen with the chain at [intake](/concepts/vocabulary#intake). The profile's
...
`profile: strong` (`sonnet` at `high` effort). An install seeded before profiles
existed keeps its own files. `kraft admin doctor` lists the missing `profiles:`
section under what the install is missing.
