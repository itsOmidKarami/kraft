...
- The work item's `set-overrides` values and a node's `model`/`effort` override
...
rate limit, which is `claude`, `codex` and `opencode`. `gemini`, `amp`
...
[`escalation_harness`](/reference/configuration/policy#defaults-and-maxima). A gate's automated review is not
