...
approve`. See [Agent integration](/reference/agent-integration) if that's closer
...
way — see [Install](install) for that path, the install script, and
running from source.
...
- **[Concepts](concepts)** — the vocabulary this walkthrough used: chain,
  node, task, gate, cap.
- **[Configuration](configuration)** — every field in `library.yaml`,
  `repos.yaml`, `policy.yaml`, `access.yaml`.
- **[Agent integration](agent-integration)** — doing all of the above from
  inside a coding-agent session instead of this shell.
- **[Remote access](remote-access)** — approving that gate from your phone.
