...
| `grants` | only accumulates; a workspace meet keeps only what every repository grants; the work item's own override can drop one, never add one | Named operations a task is guaranteed: `git-commit`, `git-rebase`, `git-push`. The [permission gate](permissions.md#grants) allows a call that is exactly one plain git invocation of a granted subcommand, whatever `allowed_tools` says, unless `deny_tools` denies it. On Cursor the gate's allow does not override Cursor's own classifier. |
