...
// index: the picker, the chain preview, the header's
...
test("intake from an existing plan, end to end", async ({ page }) => {
  ...
  await expect(modal.getByLabel("spec")).toHaveValue(".engineering/specs/ws.md");
  await expect(modal.locator(".chain-preview")).toBeVisible();
