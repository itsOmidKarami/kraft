...
const HARNESSES: Harnesses = {
  ...
  agent_profiles: [
    ...
    },
    {
      ...
      chains: ["default"],
      problems: [
        "chain 'default' task 'q.main.t': profile 'fast' has no model for provider 'codex' (harness 'codex')",
      ],
    },
  ],
};
...
const cap = (
  values: string[] = [],
  always: string | string[] | null = null,
  extra: Partial<HarnessCapability> = {},
) => ({
  cli: [],
  values,
  always,
  ...
  valid: {
    claude: {
      ...
      capabilities: {
        ...
        effort: cap(["low", "high", "max"], null, {
          cli: ["--effort", "{value}"],
        }),
        deny_tools: cap([], ["Monitor", "Agent"], {
          cli: ["--disallowed-tools", "{csv}"],
        }),
...
const row = (name: string) =>
  screen.getByText(name, { selector: ".capability-name" }).closest("li")!;
...
describe("Settings · harnesses", () => {
  it("lists every profile with how many library tasks select it", async () => {
    ...
    expect(
      screen.getByText(/gemini: gemini.yaml: unknown capability/),
    ).toBeInTheDocument();
  });
  ...
  it("a profile links to the library tasks and chains that select it", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "tasks.implementer" }),
    ).toHaveAttribute("href", "/settings/library?c=tasks.implementer");
    expect(screen.getByRole("link", { name: "default" })).toHaveAttribute(
      "href",
      "/settings/chains?tpl=default",
    );
  });
  ...
  it("shows the selected profile's provider capabilities, read-only", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "provider codex" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByLabelText(/default permission_mode/),
  ...
  it("each capability row says what it means and the flag it becomes (Kraft-ok48k)", async () => {
    ...
    expect(pm).toHaveTextContent(
      "How the CLI decides whether a tool call needs approval",
    );
    ...
    expect(row("usage")).toHaveTextContent(
      "read from the output stream (claude-stream-json)",
    );
  ...
  it("names the packaged definition's path as that, and says how to override it", async () => {
    ...
    expect(
      await screen.findByText(
        "Packaged definition: /pkg/harnesses/claude.yaml",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /drop a claude.yaml into \$KRAFT_HOME\/templates\/harnesses\//,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("an override from $KRAFT_HOME is labelled as yours", async () => {
    ...
    expect(
      await screen.findByText(
        "Your override: /home/templates/harnesses/codex.yaml",
      ),
    ).toBeInTheDocument();
  ...
  it("saves the edited profile, dropping an emptied default and an empty executable", async () => {
    const put = vi
      .spyOn(api, "putHarness")
      .mockResolvedValue(HARNESSES.profiles[0]);
    renderAt("/settings/harnesses?h=claude");
    ...
    await userEvent.click(
      screen.getByRole("switch", { name: "disable claude" }),
    );
    await userEvent.click(screen.getByRole("button", { name: "Save" }));
    expect(put).toHaveBeenCalledWith("claude", {
      provider: "claude",
      enabled: false,
  ...
  it("a save the server refuses names why and keeps the draft", async () => {
    vi.spyOn(api, "putHarness").mockRejectedValue(
      new Error(
        "chain 'default' task 'impl.main.implementer': profile 'claude' is disabled",
      ),
    );
    renderAt("/settings/harnesses?h=claude");
    await userEvent.click(
      await screen.findByRole("switch", { name: "disable claude" }),
    );
    await userEvent.click(screen.getByRole("button", { name: "Save" }));
    expect(
      await screen.findByText(/profile 'claude' is disabled/),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("switch", { name: "enable claude" }),
    ).toBeInTheDocument();
  });
  ...
  it("changing the provider offers that provider's defaults", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText(/default effort/)).toBeInTheDocument(),
    );
  ...
  it("lists the agent profiles under the harnesses read-only with any pairing problem", async () => {
    ...
    expect(strong.querySelector("a")).toHaveAttribute(
      "href",
      "/settings/library?c=tasks.repair",
    );
    expect(screen.getByText(/Edit them in harnesses.yaml/)).toBeInTheDocument();
    const fast = screen
      .getByText("fast", { selector: ".capability-name" })
      .closest("li")!;
