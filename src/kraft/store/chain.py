...
def revise_chain(conn: sqlite3.Connection, work_item_id, revised: str, payload: dict) -> None:
    ...
    """
    forked = conn.execute(
        "SELECT run_chain IS NOT NULL FROM work_items WHERE id = ?", (work_item_id,)
    ).fetchone()[0]
    column = "run_chain" if forked else "materialized_chain"
    conn.execute(
