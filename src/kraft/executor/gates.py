...
async def review_gates(
    ...
    *,
    ...
    while status == "awaiting_gate":
        ...
        # feature.
        reviewer = node.auto_review.path if node.auto_review is not None else None
        if stops.budget_breach(db, work_item_id, budget, row=row, path=reviewer) is not None:
            await db.write(
